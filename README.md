...
同一个主题（贴纸波普）、同一个界面：左边只把**颜色变量**搬了过去，右边连**结构件**一起挂上了。

| | | |
|---|---|---|
| ![只给颜色](previews/cola/07-同一个主题-只给颜色.png) | ![带上结构件](previews/cola/08-同一个主题-带上结构件.png) | ![差异图](previews/cola/09-差异图-白的就是结构件带来的.png) |
| 只给颜色：色对上了，形状还是宿主原来的 | 带上结构件：描边、分区、选中态都进来了 | 白的就是两者不同的地方 —— 全是描边和结构线 |

逐像素量过：整屏 **0.81%** 不同（侧栏 **4.63%**、主区 **0.46%**），差异集中在中部和侧栏的**描边与结构分隔线**上。

**为什么会这样**：颜色走的是 CSS 变量，任何宿主都覆写得动；结构件挂的是**元素钩子**
（`.loki-sidebar`、`.loki-composer` 这类），普通宿主里没有钩子可挂，
所以别处搬来的主题常常只剩下颜色。`scripts/probe.py` + `scripts/adapt.py` 就是干这件事的：
先探出宿主的真实元素，再把钩子挂上去，升级后还能重跑一遍。

> 本节截图里会话列表打了码（新建的空白会话 + 标题模糊），不是摆拍。
